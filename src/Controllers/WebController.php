...
final class WebController {
    ...
    public function documentForm():void{Auth::require();$type=(string)($_GET['type']??'invoice');$allowed=['invoice','proforma','offer','order','credit'];if(!in_array($type,$allowed,true))$type='invoice';$tp=$this->db->prepare('SELECT * FROM tax_profiles WHERE workspace_id=?');$tp->execute([Auth::workspaceId()]);$taxProfile=$tp->fetch()?:[];$inv=$this->db->prepare('SELECT id,doc_number FROM documents WHERE workspace_id=? AND doc_type="invoice" AND status<>"cancelled" ORDER BY issue_date DESC,id DESC LIMIT 200');$inv->execute([Auth::workspaceId()]);$invoices=$inv->fetchAll();View::render('documents/form',['title'=>'Nový doklad','customers'=>$this->customersList(),'products'=>$this->productsList(),'prefillDocType'=>$type,'taxProfile'=>$taxProfile,'invoices'=>$invoices]);}
    ...
    public function documentCancel(int $id):void{Auth::require();Auth::verifyCsrf();$wid=Auth::workspaceId();$s=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$s->execute([$id,$wid]);$d=$s->fetch();if(!$d)Response::abort(404,'Doklad nenalezen.');if($d['payment_status']==='paid')Response::abort(422,'Zaplacený doklad nelze stornovat.');$this->db->beginTransaction();try{$items=$this->db->prepare('SELECT * FROM document_items WHERE document_id=? AND warehouse_item_id IS NOT NULL');$items->execute([$id]);foreach($items->fetchAll() as $it){if($d['doc_type']==='invoice'){$this->db->prepare('UPDATE products SET stock=stock+? WHERE id=? AND workspace_id=?')->execute([(float)$it['quantity'],(int)$it['warehouse_item_id'],$wid]);$this->db->prepare('INSERT INTO stock_movements(workspace_id,product_id,qty,type,note) VALUES(?,?,?,?,?)')->execute([$wid,(int)$it['warehouse_item_id'],(float)$it['quantity'],'cancel','Vrácení skladu při stornu dokladu '.$d['doc_number']]);}}$this->db->prepare("UPDATE documents SET status='cancelled',payment_status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=?")->execute([$id,$wid]);$this->db->commit();$this->audit('cancel','document',$id);}catch(\Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}Response::redirect('/documents');}
    ...
    public function documentSave():void{
        ...
        try {
            ...
            $tp=$this->db->prepare('SELECT * FROM tax_profiles WHERE workspace_id=?');$tp->execute([$wid]);$taxProfile=$tp->fetch()?:['vat_payer'=>0];$vatPayer=!empty($taxProfile['vat_payer']);
            $names=$_POST['item_name']??[]; $qty=$_POST['item_qty']??[]; $prices=$_POST['item_price']??[]; $vat=$_POST['item_vat']??[]; $productIds=$_POST['product_id']??[];
            $sub=0; $vatSum=0; $items=[];
            foreach($names as $i=>$name){
                if(trim($name)==='') continue; $q=(float)($qty[$i]??1); $price=(float)($prices[$i]??0); $vr=(float)($vat[$i]??21); $productId=(int)($productIds[$i]??0);
                if($productId>0){
                    ...
                    if(!$prod) Response::abort(422,'Skladová položka nebyla nalezena.'); if($type==='invoice' && $q>(float)$prod['stock']) Response::abort(422,'Nedostatek zboží skladem pro položku '.$prod['name'].'. K dispozici je '.$prod['stock'].'.');
                    if(trim($name)==='') $name=$prod['name'];
                    if(empty($_POST['item_unit'][$i])) $_POST['item_unit'][$i]=$prod['unit']?:'ks';
                }
                $vr=$vatPayer?max(0,$vr):0; $line=round($q*$price,2); $sub+=$line; $vatSum+=round($line*$vr/100,2);
                $items[]=[$name,$q,$_POST['item_unit'][$i]??'ks',$price,$vr,$line,$productId];
            }
            $sign=$type==='credit'?-1:1; $sub*=$sign; $vatSum*=$sign; $total=round($sub+$vatSum,2); $num=DocumentService::nextNumber($this->db,$wid,$type); $publicToken=DocumentService::publicToken($this->db);
            ...
            $paidNow=!empty($_POST['paid_now']) && in_array($type,['invoice','proforma'],true);$parentId=$type==='credit'?(int)($_POST['parent_document_id']??0):0;if($parentId){$pc=$this->db->prepare('SELECT id FROM documents WHERE id=? AND workspace_id=? AND doc_type="invoice"');$pc->execute([$parentId,$wid]);if(!$pc->fetchColumn())Response::abort(422,'Původní faktura nebyla nalezena.');}
            $paymentStatus=$paidNow?'paid':'unpaid';
            $s=$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,parent_document_id,status,payment_status,issue_date,due_date,tax_date,payment_method,tax_regime,currency,exchange_rate,total_without_vat,total_vat,total_with_vat,created_by,public_token) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
            $s->execute([$wid,$type,$num,preg_replace('/\D/','',$num),$cid,$parentId?:null,($type==='offer'?'pending':'issued'),$paymentStatus,$_POST['issue_date']??date('Y-m-d'),$_POST['due_date']??date('Y-m-d',strtotime('+14 days')),$_POST['tax_date']??($_POST['issue_date']??date('Y-m-d')),$paymentMethod,$_POST['tax_regime']??'standard',strtoupper($_POST['currency']??'CZK'),max(0.000001,(float)($_POST['exchange_rate']??1)),$sub,$vatSum,$total,Auth::id(),$publicToken]);
    ...
    public function bankImport():void{Auth::require();Auth::verifyCsrf();if(empty($_FILES['csv']['tmp_name']))Response::redirect('/bank');$fh=fopen($_FILES['csv']['tmp_name'],'r');$head=fgetcsv($fh,0,';');if(!$head)$head=fgetcsv($fh,0,',');$delimiter=count($head)>1?';':',';$map=[];foreach($head as $i=>$h){$k=mb_strtolower(trim((string)$h));$map[$k]=$i;}while(($r=fgetcsv($fh,0,$delimiter))!==false){if(count($r)<2)continue;$date=$r[$map['datum']??($map['date']??0)]??$r[0]??'';$amount=(float)str_replace(',','.',$r[$map['částka']??($map['amount']??1)]??$r[1]??0);$counter=$r[$map['protiúčet']??($map['counterparty']??3)]??'';$vs=preg_replace('/\D/','',(string)($r[$map['variabilní symbol']??($map['variable symbol']??4)]??''));$msg=$r[$map['zpráva pro příjemce']??($map['message']??5)]??'';$currency=$r[$map['měna']??($map['currency']??2)]??'CZK';$external=hash('sha256',json_encode([$date,$amount,$vs,$counter,$msg],JSON_UNESCAPED_UNICODE));$chk=$this->db->prepare('SELECT id FROM bank_transactions WHERE workspace_id=? AND (external_id=? OR (booked_at=? AND amount=? AND variable_symbol=? AND counterparty=?)) LIMIT 1');$chk->execute([Auth::workspaceId(),$external,$date,$amount,$vs,$counter]);if($chk->fetchColumn())continue;$this->db->prepare('INSERT INTO bank_transactions(workspace_id,booked_at,amount,currency,counterparty,variable_symbol,message,status,external_id,raw_json) VALUES(?,?,?,?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),$date,$amount,$currency,$counter,$vs,$msg,'unmatched',$external,json_encode($r,JSON_UNESCAPED_UNICODE)]);$id=(int)$this->db->lastInsertId();MatcherService::match($this->db,Auth::workspaceId(),$id);}fclose($fh);Response::redirect('/bank');}
    ...
    public function aiConfirm():void{
        ...
        elseif($a['action_type']==='create_invoice'){$cid=(int)$p['customer_id'];$tp=$this->db->prepare('SELECT vat_payer FROM tax_profiles WHERE workspace_id=?');$tp->execute([Auth::workspaceId()]);$vr=$tp->fetchColumn()?21:0;if(!$cid)Response::json(['ok'=>false,'error'=>'AI nenašla jednoznačného zákazníka.'],422);$n=DocumentService::nextNumber($this->db,Auth::workspaceId(),'invoice');$amount=(float)$p['amount'];$publicToken=DocumentService::publicToken($this->db);$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,status,payment_status,issue_date,due_date,total_without_vat,total_vat,total_with_vat,created_by,public_token) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),'invoice',$n,preg_replace('/\D/','',$n),$cid,'issued','unpaid',date('Y-m-d'),date('Y-m-d',strtotime('+14 days')),round($amount/(1+$vr/100),2),round($amount-round($amount/(1+$vr/100),2),2),$amount,Auth::id(),$publicToken]);$id=(int)$this->db->lastInsertId();$this->db->prepare('INSERT INTO document_items(document_id,name,quantity,unit,unit_price,vat_rate,line_total) VALUES(?,?,?,?,?,?,?)')->execute([$id,$p['description'],1,'ks',round($amount/(1+$vr/100),2),$vr,round($amount/(1+$vr/100),2)]);$this->audit('ai_create','invoice',$id,$p);}
    ...
    public function recurringSave():void{Auth::require();Auth::verifyCsrf();$cid=(int)$_POST['customer_id'];$cq=$this->db->prepare('SELECT id FROM customers WHERE id=? AND workspace_id=? AND active=1');$cq->execute([$cid,Auth::workspaceId()]);if(!$cq->fetchColumn())Response::abort(404,'Zákazník nenalezen.');$interval=$_POST['interval_type']??'month';if(!in_array($interval,['month','quarter','year'],true))Response::abort(422,'Neplatný interval.');$iv=max(1,(int)($_POST['interval_value']??1));$tp=$this->db->prepare('SELECT vat_payer FROM tax_profiles WHERE workspace_id=?');$tp->execute([Auth::workspaceId()]);$vatPayer=(bool)$tp->fetchColumn();$tpl=['items'=>[['name'=>trim((string)($_POST['item_name']??'')),'quantity'=>(float)$_POST['quantity'],'unit'=>$_POST['unit']??'ks','unit_price'=>(float)$_POST['unit_price'],'vat_rate'=>$vatPayer?(float)($_POST['vat_rate']??21):0]]];if($tpl['items'][0]['name']===''||$tpl['items'][0]['quantity']<=0)Response::abort(422,'Vyplňte název a kladné množství.');$this->db->prepare('INSERT INTO recurring_invoices(workspace_id,customer_id,interval_type,interval_value,next_run,send_email,template_json) VALUES(?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),$cid,$interval,$iv,$_POST['next_run']??date('Y-m-d'),!empty($_POST['send_email']),json_encode($tpl,JSON_UNESCAPED_UNICODE)]);Response::redirect('/recurring');}
    public function tax():void{Auth::require();$wid=Auth::workspaceId();$qyear=(int)($_GET['year']??0);$s=$this->db->prepare('SELECT * FROM tax_profiles WHERE workspace_id=?');$s->execute([$wid]);$profile=$s->fetch()?:[];$year=$qyear>2000?$qyear:(int)($profile['year']??date('Y'));$from=$year.'-01-01';$to=($year+1).'-01-01';$vatPayer=!empty($profile['vat_payer']);$method=$profile['income_tax_method']??'actual';if($method==='cash'){$s=$this->db->prepare('SELECT COALESCE(SUM(CASE WHEN d.doc_type="credit" THEN -p.amount ELSE p.amount END),0) FROM payments p JOIN documents d ON d.id=p.document_id WHERE p.workspace_id=? AND p.paid_at>=? AND p.paid_at<? AND d.status<>"cancelled"');$s->execute([$wid,$from,$to]);$income=(float)$s->fetchColumn();}else{$s=$this->db->prepare('SELECT COALESCE(SUM(CASE WHEN doc_type="credit" THEN -total_without_vat ELSE total_without_vat END),0) FROM documents WHERE workspace_id=? AND issue_date>=? AND issue_date<? AND status<>"cancelled" AND doc_type IN ("invoice","credit")');$s->execute([$wid,$from,$to]);$income=(float)$s->fetchColumn();}$s=$this->db->prepare('SELECT COALESCE(SUM(amount),0) FROM expenses WHERE workspace_id=? AND expense_date>=? AND expense_date<?');$s->execute([$wid,$from,$to]);$expenses=(float)$s->fetchColumn();$s=$this->db->prepare('SELECT COALESCE(SUM(CASE WHEN doc_type="credit" THEN -total_vat ELSE total_vat END),0) FROM documents WHERE workspace_id=? AND issue_date>=? AND issue_date<? AND status<>"cancelled" AND doc_type IN ("invoice","credit")');$s->execute([$wid,$from,$to]);$vatOut=$vatPayer?(float)$s->fetchColumn():0;$s=$this->db->prepare('SELECT COALESCE(SUM(vat_amount),0) FROM expenses WHERE workspace_id=? AND expense_date>=? AND expense_date<?');$s->execute([$wid,$from,$to]);$vatIn=$vatPayer?(float)$s->fetchColumn():0;$estimate=TaxService::estimate(['income'=>$income,'expenses'=>$expenses,'vat'=>max(0,$vatOut-$vatIn),'social_rate'=>$profile['social_rate']??0,'health_rate'=>$profile['health_rate']??0,'income_tax_method'=>$method,'flat_regime'=>$profile['flat_regime']??0,'flat_monthly_tax'=>$profile['flat_monthly_tax']??0,'flat_social_monthly'=>$profile['flat_social_monthly']??0,'flat_health_monthly'=>$profile['flat_health_monthly']??0,'tax_credit'=>$profile['tax_credit']??0,'threshold'=>$profile['tax_threshold']??1762812,'expense_lump_rate'=>$profile['expense_lump_rate']??0.6,'year'=>$year]);$estimate['vat_output']=$vatOut;$estimate['vat_input']=$vatIn;$estimate['vat_due']=$vatOut-$vatIn;$estimate['orientation_only']=true;View::render('tax/index',['title'=>'Daně a odvody','estimate'=>$estimate,'profile'=>$profile,'selectedYear'=>$year]);}
    public function taxSave():void{Auth::require();Auth::verifyCsrf();$this->db->prepare('INSERT INTO tax_profiles(workspace_id,vat_payer,income_tax_method,social_rate,health_rate,flat_regime,flat_monthly_tax,flat_social_monthly,flat_health_monthly,tax_credit,tax_threshold,expense_lump_rate,year) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(workspace_id) DO UPDATE SET vat_payer=excluded.vat_payer,income_tax_method=excluded.income_tax_method,social_rate=excluded.social_rate,health_rate=excluded.health_rate,flat_regime=excluded.flat_regime,flat_monthly_tax=excluded.flat_monthly_tax,flat_social_monthly=excluded.flat_social_monthly,flat_health_monthly=excluded.flat_health_monthly,tax_credit=excluded.tax_credit,tax_threshold=excluded.tax_threshold,expense_lump_rate=excluded.expense_lump_rate,year=excluded.year,updated_at=CURRENT_TIMESTAMP')->execute([Auth::workspaceId(),!empty($_POST['vat_payer'])?'1':'0',$_POST['income_tax_method']??'actual',(float)($_POST['social_rate']??0),(float)($_POST['health_rate']??0),!empty($_POST['flat_regime'])?'1':'0',(float)($_POST['flat_monthly_tax']??0),(float)($_POST['flat_social_monthly']??0),(float)($_POST['flat_health_monthly']??0),(float)($_POST['tax_credit']??0),(float)($_POST['tax_threshold']??1762812),(float)($_POST['expense_lump_rate']??0.6),(int)($_POST['year']??date('Y'))]);Response::redirect('/tax');}
    public function receivedInvoices():void{Auth::require();$rows=$this->scope('SELECT * FROM received_invoices WHERE workspace_id=? ORDER BY due_date ASC,id DESC');View::render('received-invoices/index',['title'=>'Přijaté faktury','rows'=>$rows]);}
    public function receivedInvoiceSave():void{Auth::require();Auth::verifyCsrf();$this->db->prepare('INSERT INTO received_invoices(workspace_id,supplier_name,supplier_ico,document_number,variable_symbol,issue_date,due_date,amount,vat_amount,currency,note) VALUES(?,?,?,?,?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),trim((string)($_POST['supplier_name']??'')),trim((string)($_POST['supplier_ico']??'')),trim((string)($_POST['document_number']??'')),preg_replace('/\D/','',(string)($_POST['variable_symbol']??'')),$_POST['issue_date']??null,$_POST['due_date']??null,(float)($_POST['amount']??0),(float)($_POST['vat_amount']??0),strtoupper($_POST['currency']??'CZK'),trim((string)($_POST['note']??''))?:null]);Response::redirect('/received-invoices');}
    public function receivedInvoicePaid(int $id):void{Auth::require();Auth::verifyCsrf();$this->db->prepare('UPDATE received_invoices SET payment_status="paid",paid_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=?')->execute([$id,Auth::workspaceId()]);Response::redirect('/received-invoices');}
    public function export():void{
        ...
        if($format==='pohoda'){ $rows=$this->exportRows($map[$type][1]); $xml='<?xml version="1.0" encoding="UTF-8"?><dat:dataPack xmlns:dat="http://www.stormware.cz/schema/version_2/data.xsd" version="2.0"><dat:dataPackItem id="byznio-'.date('YmdHis').'" version="2.0"><lst:lstInvoice xmlns:lst="http://www.stormware.cz/schema/version_2/list.xsd" xmlns:inv="http://www.stormware.cz/schema/version_2/invoice.xsd" xmlns:typ="http://www.stormware.cz/schema/version_2/type.xsd"><lst:invoice><inv:invoiceHeader><inv:number><typ:numberRequested>'.$this->xml((string)($rows[0]['doc_number']??'' )).'</typ:numberRequested></inv:number><inv:date>'.$this->xml((string)($rows[0]['issue_date']??date('Y-m-d'))).'</inv:date></inv:invoiceHeader></lst:invoice></lst:lstInvoice></dat:dataPackItem></dat:dataPack>';header('Content-Type: application/xml; charset=UTF-8');header('Content-Disposition: attachment; filename=byznio-pohoda-'.date('Y-m-d').'.xml');echo $xml;exit;}

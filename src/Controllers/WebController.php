...
final class WebController {
    ...
    public function registerPost():void{
        ...
        if(!filter_var($email,FILTER_VALIDATE_EMAIL)){View::render('auth/register',['title'=>'Začít zdarma','error'=>'Zadejte platnou e-mailovou adresu.']);return;}
        $token=bin2hex(random_bytes(32));$hash=hash('sha256',$token);$expires=(new \DateTimeImmutable('+24 hours'))->format('Y-m-d H:i:s');
        try{$this->db->beginTransaction();$local=$this->makeEmailLocalpart($company);$this->db->prepare('INSERT INTO workspaces(name,plan,status,email_localpart) VALUES(?,?,?,?)')->execute([$company,'all','trial',$local]);
            $wid=(int)$this->db->lastInsertId();$this->db->prepare('INSERT INTO users(workspace_id,name,email,password_hash,role,email_verification_token_hash,email_verification_expires_at) VALUES(?,?,?,?,?,?,?)')->execute([$wid,$name,$email,password_hash($pass,PASSWORD_DEFAULT),'owner',$hash,$expires]);$uid=(int)$this->db->lastInsertId();
            ...
            $url=rtrim(Env::get('APP_URL',''),'/').'/verify-email?token='.urlencode($token);$subject='Potvrďte e-mail a aktivujte Byznio';$body="Dobrý den,\n\nvítejte v Byzniu. Pro dokončení registrace prosím potvrďte svou e-mailovou adresu kliknutím na aktivační tlačítko.\n\nOdkaz je platný 24 hodin. Pokud jste registraci neprováděli vy, tento e-mail ignorujte.\n\nS pozdravem,\nByznio";$ok=MailerService::send($email,$subject,$body,null,null,'Byznio',$url,null,null,null);
            $_SESSION['byznio_pending_verification_email']=$email;$_SESSION['byznio_verification_mail_sent']=$ok;$_SESSION['byznio_new_registration']=1;View::render('auth/verify-email',['title'=>'Potvrďte e-mail','email'=>$email,'sent'=>$ok]);
    ...
    public function verifyEmail():void{
        $token=(string)($_GET['token']??'');if($token===''){View::render('auth/verify-email',['title'=>'Potvrďte e-mail','error'=>'Aktivační odkaz není platný.']);return;}
        $hash=hash('sha256',$token);$s=$this->db->prepare('SELECT * FROM users WHERE email_verification_token_hash=? AND active=1 LIMIT 1');$s->execute([$hash]);$u=$s->fetch();
        if(!$u){View::render('auth/verify-email',['title'=>'Potvrďte e-mail','error'=>'Aktivační odkaz je neplatný nebo už byl použit.']);return;}
        if(empty($u['email_verification_expires_at'])||strtotime($u['email_verification_expires_at'])<time()){$_SESSION['byznio_pending_verification_email']=$u['email'];View::render('auth/verify-email',['title'=>'Odkaz vypršel','email'=>$u['email'],'error'=>'Aktivační odkaz už vypršel. Požádejte o nový odkaz.']);return;}
        $this->db->prepare('UPDATE users SET email_verified_at=CURRENT_TIMESTAMP,email_verification_token_hash=NULL,email_verification_expires_at=NULL WHERE id=?')->execute([(int)$u['id']]);Auth::login($u);$_SESSION['byznio_new_registration']=1;unset($_SESSION['byznio_pending_verification_email']);Response::redirect('/uvod');
    }
    public function resendVerification():void{
        Auth::verifyCsrf();$email=strtolower(trim($_POST['email']??($_SESSION['byznio_pending_verification_email']??'')));if(!filter_var($email,FILTER_VALIDATE_EMAIL)){View::render('auth/verify-email',['title'=>'Potvrďte e-mail','error'=>'Zadejte platný e-mail.']);return;}
        $s=$this->db->prepare('SELECT id,name,email FROM users WHERE email=? AND active=1 LIMIT 1');$s->execute([$email]);$u=$s->fetch();if(!$u||!empty($u['email_verified_at'])){View::render('auth/verify-email',['title'=>'Potvrďte e-mail','email'=>$email,'sent'=>true]);return;}
        $token=bin2hex(random_bytes(32));$this->db->prepare('UPDATE users SET email_verification_token_hash=?,email_verification_expires_at=? WHERE id=?')->execute([hash('sha256',$token),(new \DateTimeImmutable('+24 hours'))->format('Y-m-d H:i:s'),(int)$u['id']]);$url=rtrim(Env::get('APP_URL',''),'/').'/verify-email?token='.urlencode($token);$body="Dobrý den,\n\npožádali jste o nový aktivační odkaz pro Byznio. Kliknutím na tlačítko potvrďte svou e-mailovou adresu.\n\nOdkaz je platný 24 hodin.\n\nS pozdravem,\nByznio";$ok=MailerService::send($email,'Nový aktivační odkaz pro Byznio',$body,null,null,'Byznio',$url,null,null,null);$_SESSION['byznio_pending_verification_email']=$email;View::render('auth/verify-email',['title'=>'Potvrďte e-mail','email'=>$email,'sent'=>$ok,'resent'=>true]);
    }
    ...
    public function documentCancel(int $id):void{Auth::require();Auth::verifyCsrf();$wid=Auth::workspaceId();$s=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$s->execute([$id,$wid]);$d=$s->fetch();if(!$d)Response::abort(404,'Doklad nenalezen.');if($d['payment_status']==='paid')Response::abort(422,'Zaplacený doklad nelze stornovat.');$this->db->beginTransaction();try{$items=$this->db->prepare('SELECT warehouse_item_id,quantity FROM document_items WHERE document_id=? AND warehouse_item_id IS NOT NULL');$items->execute([$id]);$up=$this->db->prepare('UPDATE products SET stock=stock+? WHERE id=? AND workspace_id=?');$mv=$this->db->prepare('INSERT INTO stock_movements(workspace_id,product_id,qty,type,note) VALUES(?,?,?,?,?)');foreach($items as $it){$qty=(float)$it['quantity'];$up->execute([$qty,(int)$it['warehouse_item_id'],$wid]);$mv->execute([$wid,(int)$it['warehouse_item_id'],$qty,'cancel','Vrácení skladu při stornu dokladu '.$d['doc_number']]);}$this->db->prepare("UPDATE documents SET status='cancelled',payment_status='cancelled',updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=?")->execute([$id,$wid]);$this->db->commit();}catch(\Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}$this->audit('cancel','document',$id);Response::redirect('/documents');}
    ...
    public function documentSave():void{
        ...
        if(!in_array($type,['invoice','offer','order','proforma','credit','delivery'],true)) Response::abort(422,'Neplatný typ dokladu.');
        ...
        try {
            ...
            $tp=$this->db->prepare('SELECT * FROM tax_profiles WHERE workspace_id=?');$tp->execute([$wid]);$taxProfile=$tp->fetch()?:['vat_payer'=>0];$vatPayer=!empty($taxProfile['vat_payer']);$names=$_POST['item_name']??[]; $qty=$_POST['item_qty']??[]; $prices=$_POST['item_price']??[]; $vat=$_POST['item_vat']??[]; $productIds=$_POST['product_id']??[];
            $sub=0; $vatSum=0; $items=[];
            foreach($names as $i=>$name){
                if(trim($name)==='') continue; $q=(float)($qty[$i]??1); $price=(float)($prices[$i]??0); $vr=$vatPayer?(float)($vat[$i]??0):0;if(!in_array($vr,[0,12,21],true))$vr=0; $productId=(int)($productIds[$i]??0);
                if($productId>0){
                    $ps=$this->db->prepare('SELECT id,name,unit,stock FROM products WHERE id=? AND workspace_id=? AND active=1');$ps->execute([$productId,$wid]);$prod=$ps->fetch();
                    if(!$prod) Response::abort(422,'Skladová položka nebyla nalezena.');
                    if(trim($name)==='') $name=$prod['name'];
                    if(empty($_POST['item_unit'][$i])) $_POST['item_unit'][$i]=$prod['unit']?:'ks';
                }
                if($productId>0 && $type==='invoice'){$stock=(float)$prod['stock'];if($q>$stock)Response::abort(422,'Nedostatek zásoby pro položku '.$prod['name'].'. K dispozici: '.$stock.'.');}$line=round($q*$price,2); $sub+=$line; $vatSum+=round($line*$vr/100,2);
                $items[]=[$name,$q,$_POST['item_unit'][$i]??'ks',$price,$vr,$line,$productId];
            }
            ...
            $s=$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,parent_document_id,status,payment_status,issue_date,due_date,tax_date,payment_method,total_without_vat,total_vat,total_with_vat,currency,cnb_rate,tax_regime,language,created_by,public_token) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
            $s->execute([$wid,$type,$num,preg_replace('/\D/','',$num),$cid,($type==='credit'?(int)($_POST['parent_document_id']??0)?:null:null),($type==='offer'?'pending':'issued'),$paymentStatus,$_POST['issue_date']??date('Y-m-d'),$_POST['due_date']??date('Y-m-d',strtotime('+14 days')),($_POST['tax_date']??($_POST['issue_date']??date('Y-m-d'))),$paymentMethod,$type==='credit'?-abs($sub):$sub,$type==='credit'?-abs($vatSum):$vatSum,$type==='credit'?-abs($total):$total,strtoupper($_POST['currency']??'CZK'),(float)($_POST['cnb_rate']??1),$_POST['tax_regime']??'standard',in_array($_POST['language']??'cs',['cs','en'],true)?$_POST['language']:'cs',Auth::id(),$publicToken]);
            $id=(int)$this->db->lastInsertId();
            $iStmt=$this->db->prepare('INSERT INTO document_items(document_id,name,quantity,unit,unit_price,vat_rate,line_total,warehouse_item_id) VALUES(?,?,?,?,?,?,?,?)');
            $mStmt=$this->db->prepare('INSERT INTO stock_movements(workspace_id,product_id,qty,type,note) VALUES(?,?,?,?,?)');
            foreach($items as $it){
                $iStmt->execute([$id,$it[0],$it[1],$it[2],$it[3],$it[4],$it[5],$it[6]?:null]);
                if($it[6]>0 && $type==='invoice'){
                    $mStmt->execute([$wid,$it[6],-$it[1],'invoice','Výdej ze skladu při vystavení faktury '.$num]);
                    $this->db->prepare('UPDATE products SET stock=stock-? WHERE id=? AND workspace_id=?')->execute([$it[1],$it[6],$wid]);
                }
            }
    ...
    public function bankImport():void{Auth::require();Auth::verifyCsrf();if(empty($_FILES['csv']['tmp_name']))Response::redirect('/bank');$rows=BankCsvParser::parse($_FILES['csv']['tmp_name']);$wid=Auth::workspaceId();$insert=$this->db->prepare('INSERT INTO bank_transactions(workspace_id,bank_account_id,booked_at,amount,currency,counterparty,account_number,variable_symbol,constant_symbol,specific_symbol,message,status,external_id,raw_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)');$check=$this->db->prepare('SELECT id FROM bank_transactions WHERE workspace_id=? AND external_id=? LIMIT 1');$count=0;foreach($rows as $r){$check->execute([$wid,$r['external_id']]);if($check->fetchColumn())continue;$insert->execute([$wid,(int)($_POST['bank_account_id']??0)?:null,$r['booked_at'],$r['amount'],$r['currency'],$r['counterparty'],$r['account_number'],$r['variable_symbol'],$r['constant_symbol'],$r['specific_symbol'],$r['message'],'unmatched',$r['external_id'],json_encode($r['raw'],JSON_UNESCAPED_UNICODE)]);$id=(int)$this->db->lastInsertId();MatcherService::match($this->db,$wid,$id);$count++;}$_SESSION['bank_import_count']=$count;Response::redirect('/bank');}
    ...
    public function aiConfirm():void{
        ...
        elseif($a['action_type']==='create_invoice'){$cid=(int)$p['customer_id'];if(!$cid)Response::json(['ok'=>false,'error'=>'AI nenašla jednoznačného zákazníka.'],422);$n=DocumentService::nextNumber($this->db,Auth::workspaceId(),'invoice');$amount=(float)$p['amount'];$tp=$this->db->prepare('SELECT vat_payer FROM tax_profiles WHERE workspace_id=?');$tp->execute([Auth::workspaceId()]);$vatPayer=(bool)$tp->fetchColumn();$tp2=$this->db->prepare('SELECT default_vat_rate FROM tax_profiles WHERE workspace_id=?');$tp2->execute([Auth::workspaceId()]);$rate=$vatPayer?(float)($tp2->fetchColumn()?:21):0;$publicToken=DocumentService::publicToken($this->db);$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,status,payment_status,issue_date,due_date,total_without_vat,total_vat,total_with_vat,created_by,public_token) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),'invoice',$n,preg_replace('/\D/','',$n),$cid,'issued','unpaid',date('Y-m-d'),date('Y-m-d',strtotime('+14 days')),$amount,0,$amount,Auth::id(),$publicToken]);$id=(int)$this->db->lastInsertId();$this->db->prepare('INSERT INTO document_items(document_id,name,quantity,unit,unit_price,vat_rate,line_total) VALUES(?,?,?,?,?,?,?)')->execute([$id,$p['description'],1,'ks',$amount,$rate,$amount]);$this->audit('ai_create','invoice',$id,$p);}
    ...
    public function settingsSave():void{
        ...
        $seriesTypes=['invoice'=>'FV','offer'=>'CN','order'=>'OBJ','proforma'=>'ZAL','credit'=>'DOB','delivery'=>'DL'];foreach($seriesTypes as $type=>$defaultPrefix){$prefix=trim((string)($_POST['series_prefix_'.$type]??''));$next=max(1,(int)($_POST['series_next_'.$type]??1));if($prefix!==''){$prefix=preg_replace('/[^A-Za-z0-9_-]/','',$prefix);if($prefix==='')$prefix=$defaultPrefix;$this->db->prepare('INSERT INTO document_series(workspace_id,doc_type,prefix,next_number,series_year,year_prefix) VALUES(?,?,?,?,?,?) ON CONFLICT(workspace_id,doc_type) DO UPDATE SET prefix=excluded.prefix,next_number=excluded.next_number,year_prefix=excluded.year_prefix')->execute([Auth::workspaceId(),$type,$prefix,$next,(int)date('Y'),$prefix]);}}
    ...
    public function bankSaltEdgeCallback():void{
        Auth::require();
        try{$customer=$this->db->prepare('SELECT customer_ref FROM bank_connections WHERE workspace_id=? AND provider="saltedge" LIMIT 1');$customer->execute([Auth::workspaceId()]);$ref=$customer->fetchColumn();if(!$ref)throw new \RuntimeException('Salt Edge customer nebyl nalezen.');$j=SaltEdgeService::connections((string)$ref);$rows=$j['data']??[];foreach($rows as $c){$existing=$this->db->prepare('SELECT id FROM bank_connections WHERE workspace_id=? AND provider="saltedge" AND connection_ref=? LIMIT 1');$existing->execute([Auth::workspaceId(),(string)($c['id']??'')]);$localId=$existing->fetchColumn();
                if($localId){$this->db->prepare('UPDATE bank_connections SET status=?,meta_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=?')->execute([$c['status']??'active',json_encode($c,JSON_UNESCAPED_UNICODE),(int)$localId,Auth::workspaceId()]);}
                else{$this->db->prepare('INSERT INTO bank_connections(workspace_id,provider,customer_ref,connection_ref,status,meta_json) VALUES(?,?,?,?,?,?)')->execute([Auth::workspaceId(),'saltedge',(string)$ref,(string)($c['id']??''),$c['status']??'active',json_encode($c,JSON_UNESCAPED_UNICODE)]);}}Response::redirect('/bank/accounts');}catch(\Throwable $e){Response::redirect('/bank/accounts?error='.rawurlencode('Open Banking: '.$e->getMessage()));}
    }
    public function bankSaltEdgeSync(int $id):void{
        Auth::require();Auth::verifyCsrf();$s=$this->db->prepare('SELECT * FROM bank_connections WHERE id=? AND workspace_id=? AND provider="saltedge"');$s->execute([$id,Auth::workspaceId()]);$c=$s->fetch();if(!$c)Response::abort(404,'Bankovní připojení nenalezeno.');try{
            ...
            $acc=SaltEdgeService::accounts($c['connection_ref']);foreach(($acc['data']??[]) as $a){$this->db->prepare('UPDATE bank_connections SET account_ref=?,account_name=?,iban=?,currency=? WHERE id=?')->execute([(string)$a['id'],$a['name']??null,$a['extra']['iban']??null,$a['currency_code']??'CZK',$id]);$tx=SaltEdgeService::transactions($c['connection_ref'],(string)$a['id']);foreach(($tx['data']??[]) as $r){$amount=(float)($r['amount']??0);$date=$r['made_on']??date('Y-m-d');$external=(string)($r['id']??sha1(json_encode($r)));$desc=(string)($r['description']??$r['information']??'');$extra=is_array($r['extra']??null)?$r['extra']:[];$vs=(string)($r['variable_symbol']??$extra['variable_symbol']??'');$acc=(string)($r['account_number']??$extra['account_number']??'');$refTx=(string)($r['end_to_end_id']??$r['reference']??$extra['reference']??'');$ins=$this->db->prepare('INSERT OR IGNORE INTO bank_transactions(workspace_id,bank_account_id,booked_at,amount,currency,counterparty,account_number,variable_symbol,constant_symbol,specific_symbol,reference,message,status,external_id,raw_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');$ins->execute([Auth::workspaceId(),$bankId,$date,$amount,$r['currency_code']??'CZK',$desc,$acc,preg_replace('/\D/','',$vs),'','','',$refTx,$desc,'unmatched',$external,json_encode($r,JSON_UNESCAPED_UNICODE)]);if($ins->rowCount()){ $tid=(int)$this->db->lastInsertId();MatcherService::match($this->db,Auth::workspaceId(),$tid);}}}$this->db->prepare('UPDATE bank_connections SET last_sync_at=CURRENT_TIMESTAMP,last_error=NULL,status="active" WHERE id=?')->execute([$id]);$msg='Multi-bank synchronizace dokončena.';}catch(\Throwable $e){$this->db->prepare('UPDATE bank_connections SET last_error=? WHERE id=?')->execute([$e->getMessage(),$id]);$msg='Synchronizace selhala: '.$e->getMessage();}Response::redirect('/bank/accounts');
    }
    ...
    public function recurringSave():void{Auth::require();Auth::verifyCsrf();$cid=(int)$_POST['customer_id'];$cq=$this->db->prepare('SELECT id FROM customers WHERE id=? AND workspace_id=? AND active=1');$cq->execute([$cid,Auth::workspaceId()]);if(!$cq->fetchColumn())Response::abort(404,'Zákazník nenalezen.');$interval=$_POST['interval_type']??'month';if(!in_array($interval,['month','quarter','year'],true))Response::abort(422,'Neplatný interval.');$iv=max(1,(int)($_POST['interval_value']??1));$tp=$this->db->prepare('SELECT vat_payer FROM tax_profiles WHERE workspace_id=?');$tp->execute([Auth::workspaceId()]);$payer=(bool)$tp->fetchColumn();$rate=$payer?(float)($_POST['vat_rate']??0):0;if(!in_array($rate,[0,12,21],true))$rate=0;$tpl=['items'=>[['name'=>trim((string)($_POST['item_name']??'')),'quantity'=>(float)$_POST['quantity'],'unit'=>$_POST['unit']??'ks','unit_price'=>(float)$_POST['unit_price'],'vat_rate'=>$rate]]];if($tpl['items'][0]['name']===''||$tpl['items'][0]['quantity']<=0)Response::abort(422,'Vyplňte název a kladné množství.');$this->db->prepare('INSERT INTO recurring_invoices(workspace_id,customer_id,interval_type,interval_value,next_run,send_email,template_json) VALUES(?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),$cid,$interval,$iv,$_POST['next_run']??date('Y-m-d'),!empty($_POST['send_email']),json_encode($tpl,JSON_UNESCAPED_UNICODE)]);Response::redirect('/recurring');}
    public function tax():void{
        Auth::require();$wid=Auth::workspaceId();
        $s=$this->db->prepare('SELECT * FROM tax_profiles WHERE workspace_id=?');$s->execute([$wid]);$profile=$s->fetch()?:['year'=>(int)date('Y'),'vat_payer'=>0,'income_tax_method'=>'actual'];
        $year=max(2000,(int)($_GET['year']??$profile['year']??date('Y')));
        $estimate=TaxService::calculate($this->db,$wid,$year,$profile);
        View::render('tax/index',['title'=>'Daně a odvody','estimate'=>$estimate,'profile'=>$profile,'year'=>$year]);
    }
    ...
    public function runReminders():void{Auth::requireRole(['owner','admin']);Auth::verifyCsrf();$count=0;$rows=$this->scope('SELECT d.*,c.email,c.company_name,c.first_name,c.last_name,w.mail_enabled,w.mail_reminders FROM documents d LEFT JOIN customers c ON c.id=d.customer_id JOIN workspaces w ON w.id=d.workspace_id WHERE d.workspace_id=? AND d.doc_type="invoice" AND d.payment_status!="paid" AND d.due_date IS NOT NULL AND c.email IS NOT NULL');foreach($rows as $d){$days=(int)floor((strtotime(date('Y-m-d'))-strtotime($d['due_date']))/86400);$send=in_array($days,[-3,0,3,7],true) && !empty($d['mail_enabled']) && !empty($d['mail_reminders']);if($send){$ded=$this->db->prepare('SELECT COUNT(*) FROM reminder_log WHERE workspace_id=? AND document_id=? AND days_offset=?');$ded->execute([Auth::workspaceId(),$d['id'],$days]);if((int)$ded->fetchColumn())continue;$subject=$days<0?'Blíží se splatnost faktury '.$d['doc_number']:'Upomínka k faktuře '.$d['doc_number'];$body=$days<0?'Dobrý den, připomínáme blížící se splatnost faktury '.$d['doc_number'].' ve výši '.number_format($d['total_with_vat'],2,',',' ').' Kč.':'Dobrý den, faktura '.$d['doc_number'].' je po splatnosti '.$days.' dní. Prosíme o kontrolu úhrady.';$this->db->prepare('INSERT INTO email_queue(workspace_id,to_email,subject,body,action_url,status,scheduled_at) VALUES(?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),$d['email'],$subject,$body,rtrim(Env::get('APP_URL',''),'/').'/d/'.$d['public_token'],'queued',date('Y-m-d H:i:s')]);$this->db->prepare('INSERT OR IGNORE INTO reminder_log(workspace_id,document_id,days_offset) VALUES(?,?,?)')->execute([Auth::workspaceId(),$d['id'],$days]);$count++;}}Response::redirect('/reminders?generated='.$count);}
    ...
    public function postmarkInbound(string $secret):void{
        $expected=trim((string)Env::get('POSTMARK_INBOUND_SECRET',''));if($expected===''||!hash_equals($expected,$secret)){http_response_code(404);exit;}
        $payload=json_decode((string)file_get_contents('php://input'),true)?:[];$to=[];foreach(($payload['ToFull']??[]) as $a){if(!empty($a['Email']))$to[]=strtolower(trim($a['Email']));}if(!$to&&!empty($payload['To']))$to=[strtolower(trim((string)$payload['To']))];
        $from=strtolower(trim((string)($payload['FromFull']['Email']??$payload['From']??'')));$subject=trim((string)($payload['Subject']??''));$body=(string)($payload['StrippedTextReply']??$payload['TextBody']??'');$html=(string)($payload['HtmlBody']??'');$messageId=trim((string)($payload['MessageID']??''));if($from===''||!$to){http_response_code(422);echo 'invalid';exit;}
        foreach($to as $recipient){$local=strtolower((string)strstr($recipient,'@',true));if($local==='')continue;$q=$this->db->prepare('SELECT id FROM workspaces WHERE lower(email_localpart)=? LIMIT 1');$q->execute([$local]);$wid=(int)$q->fetchColumn();if(!$wid)continue;$cq=$this->db->prepare('SELECT id FROM customers WHERE workspace_id=? AND lower(email)=? LIMIT 1');$cq->execute([$wid,$from]);$customerId=(int)($cq->fetchColumn()?:0);$docId=0;if(preg_match('/(?:faktura|invoice|fv)\s*#?\s*([A-Za-z0-9_-]+)/iu',$subject,$mm)){$dq=$this->db->prepare('SELECT id FROM documents WHERE workspace_id=? AND doc_number=? LIMIT 1');$dq->execute([$wid,$mm[1]]);$docId=(int)($dq->fetchColumn()?:0);}
            $this->db->prepare('INSERT INTO email_messages(workspace_id,customer_id,document_id,direction,from_email,to_email,subject,body,html_body,message_id) VALUES(?,?,?,?,?,?,?,?,?,?)')->execute([$wid,$customerId?:null,$docId?:null,'inbound',$from,$recipient,$subject,$body,$html,$messageId?:null]);if($customerId)$this->db->prepare('INSERT INTO customer_communications(workspace_id,customer_id,channel,direction,subject,message,created_by) VALUES(?,?,?,?,?,?,NULL)')->execute([$wid,$customerId,'email','inbound',$subject,$body]);}
        header('Content-Type: application/json');echo json_encode(['ok'=>true]);exit;
    }

    public function communication():void{Auth::require();$rows=$this->scope('SELECT m.*,c.company_name,c.first_name,c.last_name,d.doc_number FROM email_messages m LEFT JOIN customers c ON c.id=m.customer_id LEFT JOIN documents d ON d.id=m.document_id WHERE m.workspace_id=? ORDER BY m.created_at DESC LIMIT 100');View::render('communication/index',['title'=>'Komunikace','rows'=>$rows]);}
    ...
    public function automation():void{Auth::requireRole(['owner','admin']);$rows=$this->scope('SELECT * FROM automation_rules WHERE workspace_id=? ORDER BY created_at DESC');$runs=$this->scope('SELECT r.*,a.name FROM automation_runs r LEFT JOIN automation_rules a ON a.id=r.rule_id WHERE r.workspace_id=? ORDER BY r.created_at DESC LIMIT 30');View::render('automation/index',['title'=>'Automatizace','rules'=>$rows,'runs'=>$runs]);}
    ...
    public function documentEdit(int $id):void{
        Auth::require();$wid=Auth::workspaceId();$q=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$q->execute([$id,$wid]);$d=$q->fetch();if(!$d)Response::abort(404,'Doklad nenalezen.');if($d['payment_status']==='paid')Response::abort(422,'Zaplacený doklad nelze upravovat.');$i=$this->db->prepare('SELECT * FROM document_items WHERE document_id=? ORDER BY id');$i->execute([$id]);$tp=$this->db->prepare('SELECT * FROM tax_profiles WHERE workspace_id=?');$tp->execute([$wid]);View::render('documents/edit',['title'=>'Upravit doklad','document'=>$d,'items'=>$i->fetchAll(),'customers'=>$this->customersList(),'products'=>$this->productsList(),'vatPayer'=>!empty(($tp->fetch()['vat_payer']??0))]);
    }
    public function documentUpdate(int $id):void{
        Auth::require();Auth::verifyCsrf();$wid=Auth::workspaceId();$q=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$q->execute([$id,$wid]);$d=$q->fetch();if(!$d)Response::abort(404,'Doklad nenalezen.');if($d['payment_status']==='paid')Response::abort(422,'Zaplacený doklad nelze upravovat.');
        $this->db->beginTransaction();try{
            $old=$this->db->prepare('SELECT warehouse_item_id,quantity FROM document_items WHERE document_id=? AND warehouse_item_id IS NOT NULL');$old->execute([$id]);$restore=$this->db->prepare('UPDATE products SET stock=stock+? WHERE id=? AND workspace_id=?');$mv=$this->db->prepare('INSERT INTO stock_movements(workspace_id,product_id,qty,type,note) VALUES(?,?,?,?,?)');foreach($old as $r){$restore->execute([(float)$r['quantity'],(int)$r['warehouse_item_id'],$wid]);$mv->execute([$wid,(int)$r['warehouse_item_id'],(float)$r['quantity'],'edit','Vrácení skladu před úpravou dokladu '.$d['doc_number']]);}
            $this->db->prepare('DELETE FROM document_items WHERE document_id=?')->execute([$id]);$tp=$this->db->prepare('SELECT vat_payer FROM tax_profiles WHERE workspace_id=?');$tp->execute([$wid]);$payer=(bool)$tp->fetchColumn();$names=$_POST['item_name']??[];$qty=$_POST['item_qty']??[];$prices=$_POST['item_price']??[];$rates=$_POST['item_vat']??[];$pids=$_POST['product_id']??[];$sub=0;$vat=0;$st=$this->db->prepare('INSERT INTO document_items(document_id,name,quantity,unit,unit_price,vat_rate,line_total,warehouse_item_id) VALUES(?,?,?,?,?,?,?,?)');foreach($names as $i=>$name){$name=trim((string)$name);if($name==='')continue;$qv=max(0,(float)($qty[$i]??1));$price=(float)($prices[$i]??0);$rate=$payer?(float)($rates[$i]??0):0;if(!in_array($rate,[0,12,21],true))$rate=0;$pid=(int)($pids[$i]??0);if($pid){$ps=$this->db->prepare('SELECT * FROM products WHERE id=? AND workspace_id=? AND active=1');$ps->execute([$pid,$wid]);$prod=$ps->fetch();if(!$prod)Response::abort(422,'Produkt nenalezen.');if($qv>(float)$prod['stock'])Response::abort(422,'Nedostatek zásoby pro '.$prod['name'].'.');$name=$prod['name'];}$line=round($qv*$price,2);$sub+=$line;$vat+=round($line*$rate/100,2);$st->execute([$id,$name,$qv,$_POST['item_unit'][$i]??'ks',$price,$rate,$line,$pid?:null]);if($pid&&$d['doc_type']==='invoice'){$this->db->prepare('UPDATE products SET stock=stock-? WHERE id=? AND workspace_id=?')->execute([$qv,$pid,$wid]);$mv->execute([$wid,$pid,-$qv,'edit','Výdej skladu po úpravě dokladu '.$d['doc_number']]);}}
            $this->db->prepare('UPDATE documents SET issue_date=?,due_date=?,tax_date=?,total_without_vat=?,total_vat=?,total_with_vat=?,currency=?,cnb_rate=?,tax_regime=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND workspace_id=?')->execute([$_POST['issue_date']??$d['issue_date'],$_POST['due_date']??$d['due_date'],$_POST['tax_date']??($_POST['issue_date']??$d['issue_date']),$sub,$vat,$sub+$vat,strtoupper($_POST['currency']??$d['currency']??'CZK'),(float)($_POST['cnb_rate']??$d['cnb_rate']??1),$_POST['tax_regime']??$d['tax_regime']??'standard',$id,$wid]);$this->db->commit();
        }catch(\Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}Response::redirect('/documents');
    }
    public function documentDuplicate(int $id):void{
        Auth::require();Auth::verifyCsrf();$wid=Auth::workspaceId();$q=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$q->execute([$id,$wid]);$d=$q->fetch();if(!$d)Response::abort(404,'Doklad nenalezen.');$newType=$d['doc_type'];$num=DocumentService::nextNumber($this->db,$wid,$newType);$token=DocumentService::publicToken($this->db);$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,status,payment_status,issue_date,due_date,tax_date,payment_method,total_without_vat,total_vat,total_with_vat,currency,cnb_rate,tax_regime,language,created_by,public_token) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([$wid,$newType,$num,preg_replace('/\D/','',$num),$d['customer_id'],'issued','unpaid',date('Y-m-d'),date('Y-m-d',strtotime('+14 days')),date('Y-m-d'),$d['payment_method'],$d['total_without_vat'],$d['total_vat'],$d['total_with_vat'],$d['currency']??'CZK',$d['cnb_rate']??1,$d['tax_regime']??'standard',$d['language']??'cs',Auth::id(),$token]);$new=(int)$this->db->lastInsertId();$i=$this->db->prepare('SELECT * FROM document_items WHERE document_id=?');$i->execute([$id]);$st=$this->db->prepare('INSERT INTO document_items(document_id,name,description,quantity,unit,unit_price,vat_rate,line_total,warehouse_item_id) VALUES(?,?,?,?,?,?,?,?,?)');foreach($i as $r)$st->execute([$new,$r['name'],$r['description'],$r['quantity'],$r['unit'],$r['unit_price'],$r['vat_rate'],$r['line_total'],null]);Response::redirect('/documents/'.$new.'/edit');
    }
    public function convertProforma(int $id):void{
        Auth::require();Auth::verifyCsrf();$wid=Auth::workspaceId();$q=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$q->execute([$id,$wid]);$d=$q->fetch();if(!$d||$d['doc_type']!=='proforma')Response::abort(404,'Zálohová faktura nenalezena.');$paid=DocumentService::paid($this->db,$wid,$id);$num=DocumentService::nextNumber($this->db,$wid,'invoice');$token=DocumentService::publicToken($this->db);$remaining=max(0,(float)$d['total_with_vat']-$paid);$ratio=((float)$d['total_with_vat'])>0?$remaining/(float)$d['total_with_vat']:1;$sub=round((float)$d['total_without_vat']*$ratio,2);$vat=round((float)$d['total_vat']*$ratio,2);$this->db->beginTransaction();$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,parent_document_id,status,payment_status,issue_date,due_date,tax_date,payment_method,total_without_vat,total_vat,total_with_vat,advance_applied,public_token,created_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([$wid,'invoice',$num,preg_replace('/\D/','',$num),$d['customer_id'],$id,'issued',$paid>=$d['total_with_vat']-0.01?'paid':'unpaid',date('Y-m-d'),date('Y-m-d',strtotime('+14 days')),date('Y-m-d'),$d['payment_method'],$sub,$vat,$sub+$vat,-$paid,$token,Auth::id()]);$new=(int)$this->db->lastInsertId();$i=$this->db->prepare('SELECT * FROM document_items WHERE document_id=?');$i->execute([$id]);$st=$this->db->prepare('INSERT INTO document_items(document_id,name,quantity,unit,unit_price,vat_rate,line_total,warehouse_item_id) VALUES(?,?,?,?,?,?,?,?)');foreach($i as $r){$line=round((float)$r['line_total']*$ratio,2);$st->execute([$new,$r['name'],$r['quantity'],$r['unit'],$r['unit_price'],$r['vat_rate'],$line,$r['warehouse_item_id']]);}$this->db->prepare('INSERT INTO document_chain(workspace_id,from_document_id,to_document_id,relation) VALUES(?,?,?,?)')->execute([$wid,$id,$new,'advance_to_invoice']);$this->db->commit();Response::redirect('/documents/'.$new.'/edit');
    }
    public function accountingExport():void{
        Auth::require();$wid=Auth::workspaceId();$from=$_GET['from']??date('Y-m-01');$to=$_GET['to']??date('Y-m-d');$type=$_GET['type']??'csv';if($type==='zip'){$f=AccountingExportService::package($this->db,$wid,$from,$to,dirname(__DIR__,2));header('Content-Type: application/zip');header('Content-Disposition: attachment; filename="'.basename($f).'"');readfile($f);exit;}if($type==='pohoda'){$x=AccountingExportService::pohodaXml($this->db,$wid,$from,$to);header('Content-Type: application/xml; charset=UTF-8');header('Content-Disposition: attachment; filename="pohoda_'.$from.'_'.$to.'.xml"');echo $x;exit;}$csv=AccountingExportService::csv($this->db,$wid,$from,$to);header('Content-Type: text/csv; charset=UTF-8');header('Content-Disposition: attachment; filename="byznio_'.$from.'_'.$to.'.csv"');echo "\xEF\xBB\xBF".$csv;exit;
    }
    public function receivedInvoices():void{Auth::require();$rows=$this->scope('SELECT * FROM received_invoices WHERE workspace_id=? ORDER BY due_date IS NULL,due_date DESC,id DESC');View::render('received/index',['title'=>'Přijaté faktury','rows'=>$rows]);}
    public function receivedInvoiceSave():void{Auth::require();Auth::verifyCsrf();$d=$_POST;$wid=Auth::workspaceId();$this->db->prepare('INSERT INTO received_invoices(workspace_id,supplier,ico,dic,document_number,variable_symbol,issue_date,due_date,currency,cnb_rate,amount_without_vat,vat_amount,total_amount,payment_status,note) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([$wid,trim($d['supplier']??''),trim($d['ico']??''),trim($d['dic']??''),trim($d['document_number']??''),trim($d['variable_symbol']??''),$d['issue_date']??null,$d['due_date']??null,strtoupper($d['currency']??'CZK'),(float)($d['cnb_rate']??1),(float)($d['amount_without_vat']??0),(float)($d['vat_amount']??0),(float)($d['total_amount']??0),$d['payment_status']??'unpaid',trim($d['note']??'')]);Response::redirect('/received-invoices');}
    public function isdocImport():void{Auth::require();Auth::verifyCsrf();if(empty($_FILES['isdoc']['tmp_name']))Response::abort(422,'Nahrajte ISDOC soubor.');$xml=@simplexml_load_file($_FILES['isdoc']['tmp_name']);if(!$xml)Response::abort(422,'ISDOC není validní XML.');$val=function(string $name)use($xml){$n=$xml->xpath('//*[local-name()="'.$name.'"]');return $n&&isset($n[0])?trim((string)$n[0]):'';};$id=$val('ID');$issue=$val('IssueDate')?:date('Y-m-d');$due=$val('DueDate')?:date('Y-m-d',strtotime('+14 days'));$total=(float)str_replace(',','.',$val('PayableAmount'));$name=$val('CompanyName')?:$val('PartyName');$this->db->prepare('INSERT INTO received_invoices(workspace_id,supplier,document_number,issue_date,due_date,total_amount,payment_status,note) VALUES(?,?,?,?,?,?,?,?)')->execute([Auth::workspaceId(),$name,$id,$issue,$due,$total,'unpaid','Importováno z ISDOC']);Response::redirect('/received-invoices');}
    public function accountantShare():void{Auth::require();Auth::verifyCsrf();if(!Auth::can('admin')&&!Auth::can('accountant'))Response::abort(403,'Nemáte oprávnění.');$token=bin2hex(random_bytes(24));$hash=hash('sha256',$token);$this->db->prepare('INSERT INTO accountant_shares(workspace_id,user_id,token_hash,expires_at) VALUES(?,?,?,?)')->execute([Auth::workspaceId(),Auth::id(),$hash,date('Y-m-d H:i:s',strtotime('+30 days'))]);$url=rtrim(Env::get('APP_URL',''),'/').'/accountant/'.$token;View::render('accountant/share',['title'=>'Přístup pro účetní','url'=>$url]);}
    public function comgateLink():void{Auth::require();Auth::verifyCsrf();$merchant=trim((string)($_POST['merchant']??''));$secret=trim((string)($_POST['secret']??''));if($merchant===''||$secret==='')Response::abort(422,'Doplňte Comgate údaje.');$enc=base64_encode(hash_hmac('sha256',$secret,Env::get('APP_KEY',''),true).'|'.$secret);$this->db->prepare('INSERT INTO comgate_accounts(workspace_id,merchant,secret_encrypted,active) VALUES(?,?,?,1) ON CONFLICT(workspace_id) DO UPDATE SET merchant=excluded.merchant,secret_encrypted=excluded.secret_encrypted,active=1')->execute([Auth::workspaceId(),$merchant,$enc]);Response::redirect('/settings?saved=comgate');}
    public function cnbRate():void{Auth::require();$currency=strtoupper($_GET['currency']??'EUR');$date=$_GET['date']??date('Y-m-d');$s=$this->db->prepare('SELECT rate FROM currency_rates WHERE currency=? AND rate_date=?');$s->execute([$currency,$date]);$rate=$s->fetchColumn();if($rate===false)$rate=CnbRateService::get($this->db,$currency,$date);if($rate===null)$rate=1;Response::json(['currency'=>$currency,'date'=>$date,'rate'=>(float)$rate]);}
    public function accountantPortal(string $token):void{ $hash=hash('sha256',trim($token));$q=$this->db->prepare('SELECT a.*,w.name workspace_name FROM accountant_shares a JOIN workspaces w ON w.id=a.workspace_id WHERE a.token_hash=? AND a.active=1 AND (a.expires_at IS NULL OR a.expires_at>CURRENT_TIMESTAMP)');$q->execute([$hash]);$share=$q->fetch();if(!$share)Response::abort(404,'Přístup pro účetní je neplatný nebo vypršel.');$wid=(int)$share['workspace_id'];$docs=$this->db->prepare('SELECT doc_number,doc_type,issue_date,due_date,total_with_vat,payment_status FROM documents WHERE workspace_id=? AND status<>"cancelled" ORDER BY issue_date DESC LIMIT 100');$docs->execute([$wid]);$received=$this->db->prepare('SELECT supplier,document_number,issue_date,due_date,total_amount,payment_status FROM received_invoices WHERE workspace_id=? ORDER BY issue_date DESC LIMIT 100');$received->execute([$wid]);View::renderStandalone('accountant/portal',['title'=>'Účetní portál','workspace'=>$share['workspace_name'],'documents'=>$docs->fetchAll(),'received'=>$received->fetchAll()]);}
    public function convertDocumentType(int $id,string $target):void{Auth::require();Auth::verifyCsrf();$wid=Auth::workspaceId();$allowed=['offer'=>'order','order'=>'invoice'];$q=$this->db->prepare('SELECT * FROM documents WHERE id=? AND workspace_id=?');$q->execute([$id,$wid]);$d=$q->fetch();if(!$d||($allowed[$d['doc_type']]??null)!==$target)Response::abort(422,'Tento převod není povolen.');$num=DocumentService::nextNumber($this->db,$wid,$target);$token=DocumentService::publicToken($this->db);$this->db->beginTransaction();$this->db->prepare('INSERT INTO documents(workspace_id,doc_type,doc_number,variable_symbol,customer_id,parent_document_id,status,payment_status,issue_date,due_date,tax_date,payment_method,total_without_vat,total_vat,total_with_vat,currency,cnb_rate,tax_regime,language,created_by,public_token) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)')->execute([$wid,$target,$num,preg_replace('/\D/','',$num),$d['customer_id'],$id,'issued','unpaid',date('Y-m-d'),date('Y-m-d',strtotime('+14 days')),date('Y-m-d'),$d['payment_method'],$d['total_without_vat'],$d['total_vat'],$d['total_with_vat'],$d['currency']??'CZK',$d['cnb_rate']??1,$d['tax_regime']??'standard',$d['language']??'cs',Auth::id(),$token]);$new=(int)$this->db->lastInsertId();$i=$this->db->prepare('SELECT * FROM document_items WHERE document_id=?');$i->execute([$id]);$st=$this->db->prepare('INSERT INTO document_items(document_id,name,description,quantity,unit,unit_price,vat_rate,line_total,warehouse_item_id) VALUES(?,?,?,?,?,?,?,?,?)');foreach($i as $r)$st->execute([$new,$r['name'],$r['description'],$r['quantity'],$r['unit'],$r['unit_price'],$r['vat_rate'],$r['line_total'],$target==='invoice'?$r['warehouse_item_id']:null]);$this->db->prepare('INSERT INTO document_chain(workspace_id,from_document_id,to_document_id,relation) VALUES(?,?,?,?)')->execute([$wid,$id,$new,$d['doc_type'].'_to_'.$target]);$this->db->commit();Response::redirect('/documents/'.$new.'/edit');}
    public function emailPreview(string $type='general'):void{Auth::requireRole(['owner','admin']);$types=['invoice','reminder','payment','verification','report','general'];if(!in_array($type,$types,true))$type='general';$data=['company'=>'Byznio','logo'=>rtrim(Env::get('APP_URL',''),'/').'/assets/byznio-logo.svg','title'=>['invoice'=>'Faktura FV-2026-00001','reminder'=>'Blíží se splatnost','payment'=>'Platba přijata','verification'=>'Ověření e-mailu','report'=>'Týdenní přehled','general'=>'Zpráva z Byznia'][$type],'intro'=>'Ukázkový obsah e-mailu pro kontrolu vzhledu.','action_url'=>rtrim(Env::get('APP_URL',''),'/').'/documents','action_text'=>'Otevřít v Byzniu','details'=>['Doklad'=>'FV-2026-00001','Částka'=>'12 100 Kč','Splatnost'=>'30. 9. 2026']];$r=EmailTemplateService::render($type,$data);echo $r['html'];exit;}

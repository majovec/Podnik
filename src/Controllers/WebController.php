...
final class WebController {
    ...
    public function bank():void{Auth::require();$this->gate();$tx=$this->scope('SELECT t.*,d.doc_number FROM bank_transactions t LEFT JOIN documents d ON d.id=t.matched_document_id AND d.workspace_id=t.workspace_id WHERE t.workspace_id=? ORDER BY t.booked_at DESC,t.id DESC');View::render('bank/index',['title'=>'Banka','transactions'=>$tx]);}
    ...
    public function calendar():void{Auth::require();$events=$this->scope('SELECT e.*,c.company_name,j.name job_name,u.name user_name FROM calendar_events e LEFT JOIN customers c ON c.id=e.customer_id LEFT JOIN jobs j ON j.id=e.job_id LEFT JOIN users u ON u.id=e.user_id WHERE e.workspace_id=? ORDER BY e.start_at');View::render('calendar/index',['title'=>'Kalendář','events'=>$events,'customers'=>$this->customersList(),'jobs'=>$this->scope('SELECT id,name FROM jobs WHERE workspace_id=? ORDER BY name')]);}
    ...
    public function gdprExport():void{Auth::requireRole(['owner','admin']);Auth::verifyCsrf();$wid=Auth::workspaceId();$data=[];foreach(['workspaces','users','customers','documents','document_items','payments','jobs','job_hours','job_materials','expenses','products','stock_movements','bank_accounts','bank_transactions','calendar_events','documents_files','tasks','customer_communications','audit_log'] as $table){$s=$this->db->prepare("SELECT * FROM $table WHERE workspace_id=?");$s->execute([$wid]);$data[$table]=$s->fetchAll();}header('Content-Type: application/json; charset=UTF-8');header('Content-Disposition: attachment; filename="gdpr-export-'.date('Y-m-d').'.json"');echo json_encode($data,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);exit;}
    public function exportCsv():void{Auth::requirePermission('api');$type=$_GET['type']??'invoices';$map=['invoices'=>'SELECT * FROM documents WHERE workspace_id=? ORDER BY issue_date','expenses'=>'SELECT * FROM expenses WHERE workspace_id=? ORDER BY expense_date','payments'=>'SELECT * FROM payments WHERE workspace_id=? ORDER BY paid_at'];if(!isset($map[$type]))Response::abort(400,'Neplatný export.');$s=$this->db->prepare($map[$type]);$s->execute([Auth::workspaceId()]);header('Content-Type: text/csv; charset=UTF-8');header('Content-Disposition: attachment; filename="'.$type.'-'.date('Y-m-d').'.csv"');$out=fopen('php://output','w');fwrite($out,"\xEF\xBB\xBF");$rows=$s->fetchAll();if($rows){fputcsv($out,array_keys($rows[0]),';');foreach($rows as $r)fputcsv($out,$r,';');}fclose($out);exit;}
    ...
    public function apiKeys():void{Auth::requirePermission('api');$this->gate();$rows=$this->scope('SELECT id,name,last_used_at,created_at FROM api_keys WHERE user_id=? ORDER BY created_at DESC',[Auth::id()]);View::render('settings/api-keys',['title'=>'API klíče','keys'=>$rows]);}

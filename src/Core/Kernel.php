...
final class Kernel {
    public static function handle(): void {
        ...
        $r->get('/documents',fn()=> $w->documents()); $r->get('/documents/new',fn()=> $w->documentForm()); $r->post('/documents/save',fn()=> $w->documentSave()); $r->get('/documents/{id}/pdf',fn($id)=>$w->pdf((int)$id));
        ...
        $r->get('/settings/api-keys',fn()=> $w->apiKeys()); $r->post('/settings/api-keys',fn()=> $w->apiKeyCreateWeb()); $r->get('/reminders',fn()=> $w->reminders()); $r->post('/reminders/run',fn()=> $w->runReminders()); $r->get('/admin/plans',fn()=> $w->adminPlans()); $r->get('/documents/{id}/qr',fn($id)=>$w->qr((int)$id));
        $r->get('/admin',fn()=> $w->admin()); $r->get('/admin/billing',fn()=> $w->adminBilling()); $r->post('/admin/billing/settings',fn()=> $w->saasBillingSave()); $r->post('/admin/billing/grant-free',fn()=> $w->grantFree()); $r->post('/admin/backup',fn()=> $w->backup()); $r->get('/admin/system',fn()=> $w->adminSystem()); $r->post('/admin/subscription',fn()=> $w->subscription()); $r->get('/admin/users',fn()=> $w->users()); $r->post('/admin/users',fn()=> $w->userSave());

...
final class Kernel {
    public static function handle(): void {
        ...
        $r->get('/communication',fn()=> $w->communication()); $r->post('/customers/ares',fn()=> $w->customerAres()); $r->post('/webhooks/brevo/inbound',fn()=> $w->brevoInbound()); $r->post('/webhooks/postmark/inbound/{secret}',fn($secret)=>$w->postmarkInbound((string)$secret)); $r->get('/customers',fn()=> $w->customers()); $r->get('/customers/new',fn()=> $w->customerForm()); $r->post('/customers/save',fn()=> $w->customerSave());
        ...
        $r->get('/documents',fn()=> $w->documents()); $r->get('/documents/new',fn()=> $w->documentForm()); $r->get('/documents/{id}/edit',fn($id)=>$w->documentEdit((int)$id)); $r->post('/documents/{id}/update',fn($id)=>$w->documentUpdate((int)$id)); $r->post('/documents/{id}/duplicate',fn($id)=>$w->documentDuplicate((int)$id)); $r->post('/documents/{id}/convert-proforma',fn($id)=>$w->convertProforma((int)$id)); $r->post('/documents/{id}/convert/{target}',fn($id,$target)=>$w->convertDocumentType((int)$id,(string)$target)); $r->get('/documents/create',fn()=> $w->documentForm()); $r->get('/invoice/new',fn()=> $w->documentForm()); $r->post('/documents/save',fn()=> $w->documentSave()); $r->get('/documents/{id}/pdf',fn($id)=>$w->pdf((int)$id)); $r->get('/documents/{id}/qr',fn($id)=>$w->qr((int)$id)); $r->post('/documents/{id}/send-email',fn($id)=>$w->sendDocumentEmail((int)$id));
        ...
        $r->get('/tasks',fn()=> $w->tasks()); $r->post('/tasks/save',fn()=> $w->taskSave()); $r->post('/tasks/{id}/done',fn($id)=>$w->taskDone((int)$id)); $r->get('/recurring',fn()=> $w->recurring()); $r->post('/recurring',fn()=> $w->recurringSave()); $r->get('/tax',fn()=> $tax->index()); $r->post('/tax',fn()=> $w->taxSave()); $r->get('/export',fn()=> $w->export()); $r->get('/accounting/export',fn()=> $accounting->export()); $r->get('/accountant/{token}',fn($token)=>$w->accountantPortal((string)$token)); $r->get('/received-invoices',fn()=> $received->index()); $r->post('/received-invoices/save',fn()=> $received->save()); $r->post('/received-invoices/import',fn()=> $received->import()); $r->get('/received-invoices/{id}/edit',fn($id)=>$received->edit((int)$id)); $r->post('/received-invoices/{id}/update',fn($id)=>$received->update((int)$id)); $r->get('/received-invoices/{id}/download',fn($id)=>$received->download((int)$id)); $r->post('/accountant/share',fn()=> $w->accountantShare()); $r->get('/gdpr/export',fn()=> $w->gdprExport()); $r->post('/gdpr/export',fn()=> $w->gdprExport()); $r->get('/documents/{id}/isdoc',fn($id)=>$w->isdoc((int)$id)); $r->post('/isdoc/import',fn()=> $w->isdocImport()); $r->get('/automation',fn()=> $w->automation()); $r->post('/automation/save',fn()=> $w->automationSave());
        $r->get('/email-preview/{type}',fn($type)=>$w->emailPreview((string)$type)); $r->get('/settings',fn()=> $w->settings()); $r->post('/settings/comgate',fn()=> $w->comgateLink()); $r->get('/settings/cnb-rate',fn()=> $w->cnbRate()); $r->post('/settings',fn()=> $w->settingsSave());

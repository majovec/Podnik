...
final class Kernel {
    public static function handle(): void {
        ...
        $r->get('/communication',fn()=> $w->communication()); $r->post('/webhooks/postmark/inbound/{secret}',fn($secret)=>$w->postmarkInbound((string)$secret)); $r->get('/customers',fn()=> $w->customers()); $r->get('/customers/new',fn()=> $w->customerForm()); $r->post('/customers/save',fn()=> $w->customerSave());
        ...
        $r->get('/documents',fn()=> $w->documents()); $r->get('/documents/new',fn()=> $w->documentForm()); $r->get('/documents/create',fn()=> $w->documentForm()); $r->get('/invoice/new',fn()=> $w->documentForm()); $r->post('/documents/save',fn()=> $w->documentSave()); $r->get('/documents/{id}/pdf',fn($id)=>$w->pdf((int)$id)); $r->get('/documents/{id}/qr',fn($id)=>$w->qr((int)$id));

...
final class MailerService {
    ...
    private static function shell(string $content,string $name,?string $actionUrl=null,?string $logoPath=null,?int $workspaceId=null):string{
        ...
        $footerLogo='<img src="'.htmlspecialchars($logo,ENT_QUOTES,'UTF-8').'" alt="'.$safeName.'" style="max-width:105px;max-height:34px;object-fit:contain;opacity:.92">';
        return '<!doctype html><html lang="cs"><body style="margin:0;background:#f5f8fc;font-family:Arial,sans-serif;color:#10213f"><div style="max-width:620px;margin:32px auto;padding:0 16px"><div style="background:#071a3a;padding:22px 26px;border-radius:18px 18px 0 0"><img src="'.htmlspecialchars($logo,ENT_QUOTES,'UTF-8').'" alt="'.$safeName.'" style="width:150px;max-width:100%;max-height:64px;object-fit:contain"></div><div style="background:#fff;padding:30px 26px;border:1px solid #e5eaf2;border-top:0;border-radius:0 0 18px 18px"><p style="margin-top:0">Dobrý den,</p>'.$content.$button.'<div style="margin-top:34px;padding-top:20px;border-top:1px solid #edf0f5"><p style="margin:0 0 10px;color:#4b5a70;font-size:14px">S pozdravem,<br><strong>'.$safeName.'</strong></p>'.$footerLogo.'<p style="color:#7a8799;font-size:11px;margin:8px 0 0">Odesláno prostřednictvím Byznio · stačí odpovědět na tento e-mail.</p></div></div></div></body></html>';
    }

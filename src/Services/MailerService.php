...
final class MailerService {
    ...
    private static function deliver(
        ...
        if ($token === '') {
            self::$lastError = 'BREVO_API_KEY není nastaven.';
            return false;
        }

        $from = $from ?: Env::get('MAIL_FROM');
        if (!$from) {
            self::$lastError = 'MAIL_FROM není nastaven.';
            return false;
        }
        ...
        if (!empty($GLOBALS['_byznio_mail_reply_to'])) {
            $payload['replyTo'] = ['email' => $GLOBALS['_byznio_mail_reply_to']];
            unset($GLOBALS['_byznio_mail_reply_to']);
        }

        if ($attachment) {
            $file = @file_get_contents($attachment);
            if ($file === false) {
                self::$lastError = 'Přílohu se nepodařilo načíst.';
                return false;
            }
            $payload['attachment'] = [[
        ...
        if ($response === false) {
            self::$lastError = 'Brevo cURL chyba ' . $errno . ': ' . $error;
            return false;
        }
        if ($http < 200 || $http >= 300) {
            $decoded = json_decode($response, true);
            self::$lastError = 'Brevo HTTP ' . $http . ': ' . ($decoded['message'] ?? $decoded['code'] ?? $response);
            return false;
        }
        return true;
    }

...
final class PdfService {
    ...
    public static function invoice(array $doc,array $items,array $company,array $customer): string {
        ...
        </style><div class="top"><div><div class="muted">BYZNIO</div><h1>'.$esc(self::title($doc['doc_type']??'invoice')).'</h1><div>'.$esc($doc['doc_number']).'</div></div><div><b>'.$esc($company['name']??'').'</b><br>'.$esc($company['street']??'').'<br>'.$esc(($company['zip']??'').' '.($company['city']??'')).'<br>IČO '.$esc($company['ico']??'').'</div></div>

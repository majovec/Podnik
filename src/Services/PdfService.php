...
final class PdfService {
    private static function title(string $type,string $lang='cs'): string { $cs=['invoice'=>'FAKTURA','offer'=>'NABÍDKA','order'=>'OBJEDNÁVKA','proforma'=>'ZÁLOHOVÁ FAKTURA','credit'=>'DOBROPIS','delivery'=>'DODACÍ LIST']; $en=['invoice'=>'INVOICE','offer'=>'QUOTE','order'=>'ORDER','proforma'=>'PROFORMA INVOICE','credit'=>'CREDIT NOTE','delivery'=>'DELIVERY NOTE']; return ($lang==='en'?$en:$cs)[$type] ?? ($lang==='en'?'DOCUMENT':'DOKLAD'); }
    ...
    public static function invoice(array $doc,array $items,array $company,array $customer,?string $qrUrl=null): string {
        ...
        $type=(string)($doc['doc_type']??'invoice'); $lang=(string)($doc['language']??'cs'); $paymentMethod=(string)($doc['payment_method']??'bank_transfer'); $paymentStatus=(string)($doc['payment_status']??'unpaid');
        $rows='';$vatGroups=[];
        foreach($items as $i=>$it){$rate=(float)($it['vat_rate']??0);$line=(float)($it['line_total']??0);$vatGroups[(string)$rate]=($vatGroups[(string)$rate]??0)+round($line*$rate/100,2);$rows.='<tr><td class="num">'.($i+1).'</td><td><b>'.$esc($it['name']).'</b></td><td>'.number_format((float)$it['quantity'],2,',',' ').' '.$esc($it['unit']).'</td><td class="right">'.number_format((float)$it['unit_price'],2,',',' ').' Kč</td><td class="right strong">'.number_format((float)$it['line_total'],2,',',' ').' Kč</td></tr>';}
        ...
        $taxDate=$esc($doc['tax_date']??$doc['issue_date']??'');$parent='';if(!empty($doc['parent_document_id']))$parent='<div class="smallnote">'.($lang==='en'?'Related document':'Vazba na původní doklad').': #'.$esc($doc['parent_document_id']).'</div>';
        ...
        $statusHtml=$paymentStatus==='paid'?'<span class="paid">'.($lang==='en'?'PAID':'UHRAZENO').'</span>':'';
        ...
        </style></head><body><div class="page"><div class="accent"></div><table class="top"><tr><td class="brand">'.($logoHtml?:'<div class="brandfallback">BYZNIO</div>').'<div class="eyebrow">'.($lang==='en'?'Document':'Doklad').'</div><div class="title">'.$esc(self::title($type,$lang)).'</div><div class="number">'.$esc($doc['doc_number']??'').' '.$statusHtml.'</div></td><td class="meta"><div class="meta-card"><div class="label">'.($lang==='en'?'Issued':'Vystaveno').'</div><div class="value">'.$esc($doc['issue_date']??'').'</div><div class="label" style="margin-top:9px">'.($lang==='en'?'Due date':'Splatnost').'</div><div class="value">'.$esc($doc['due_date']??'').'</div><div class="pill">'.self::paymentLabel($paymentMethod).'</div></div></td></tr></table>
        <div class="section"><table class="twocol"><tr><td class="box"><div class="cap">'.($lang==='en'?'Supplier':'Dodavatel').'</div><div class="name">'.$companyName.'</div>'.$esc($company['street']??'').'<br>'.$esc(trim((string)($company['zip']??'').' '.(string)($company['city']??''))).'<div class="ids">'.implode(' · ',$supplierIds).'</div></td><td class="box"><div class="cap">'.($lang==='en'?'Customer':'Odběratel').'</div><div class="name">'.$customerName.'</div>'.$customerAddress.'<div class="ids">'.implode(' · ',$customerIds).'</div></td></tr></table></div>
        <div class="items"><table class="items-table"><thead><tr><th>#</th><th>'.($lang==='en'?'Item':'Položka').'</th><th>'.($lang==='en'?'Qty':'Množství').'</th><th class="right">'.($lang==='en'?'Unit price':'Jedn. cena').'</th><th class="right">'.($lang==='en'?'Total':'Celkem').'</th></tr></thead><tbody>'.$rows.'</tbody></table></div>

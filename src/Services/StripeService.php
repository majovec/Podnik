...
final class StripeService {
    ...
    public static function checkout(int $workspaceId,string $price,string $success,string $cancel):?string {
        ...
        $d=['mode'=>'subscription','line_items[0][price_data][currency]'=>'czk','line_items[0][price_data][unit_amount]'=>$amount,'line_items[0][price_data][recurring][interval]'=>$interval,'line_items[0][price_data][product_data][name]'=>'Byznio','line_items[0][quantity]'=>1,'success_url'=>$success,'cancel_url'=>$cancel,'client_reference_id'=>$workspaceId,'metadata[workspace_id]'=>$workspaceId,'metadata[billing_interval]'=>$interval,'subscription_data[metadata][workspace_id]'=>$workspaceId,'subscription_data[metadata][billing_interval]'=>$interval];if($customer)$d['customer']=$customer;$x=self::request('POST','/checkout/sessions',$d);return $x['url']??null;
    }
    public static function cancelAtPeriodEnd(string $subscriptionId):array {
        return self::request('POST','/subscriptions/'.rawurlencode($subscriptionId),['cancel_at_period_end'=>'true']);
    }
    public static function billingPortal(int $workspaceId,string $returnUrl):?string {
        $pdo=Database::pdo();$st=$pdo->prepare('SELECT stripe_customer_id FROM subscriptions WHERE workspace_id=?');$st->execute([$workspaceId]);$customer=(string)($st->fetchColumn()?:'');
        if($customer==='') return null;
        $x=self::request('POST','/billing_portal/sessions',['customer'=>$customer,'return_url'=>$returnUrl]);return $x['url']??null;
    }
    ...
    public static function webhook(string $payload,string $signature):void {
        ...
        if(in_array($e['type'],['checkout.session.completed','customer.subscription.updated','customer.subscription.deleted'],true)){ $status=$e['type']==='customer.subscription.deleted'?'canceled':($obj['status']??'active');$sid=$obj['id']??($obj['subscription']??null);$customer=$obj['customer']??null;$interval=$obj['metadata']['billing_interval']??'month';$end=isset($obj['current_period_end'])?date('Y-m-d H:i:s',(int)$obj['current_period_end']):null;$cancel=(int)!empty($obj['cancel_at_period_end']);$pdo->prepare('UPDATE subscriptions SET status=?,plan="all",billing_interval=?,stripe_customer_id=COALESCE(?,stripe_customer_id),stripe_subscription_id=COALESCE(?,stripe_subscription_id),current_period_end=?,cancel_at_period_end=?,updated_at=CURRENT_TIMESTAMP WHERE workspace_id=?')->execute([$status,$interval,$customer,$sid,$end,$cancel,$wid]);$pdo->prepare('UPDATE workspaces SET plan="all",status=? WHERE id=?')->execute([$e['type']==='customer.subscription.deleted'?'trial':'active',$wid]);}
    }

...
<div class="card" style="margin-top:16px">
  ...
  <form class="form" method="post" action="/tax">
    ...
    <div class="row">
      <div class="field"><label>Daňový rok</label><input type="number" name="year" min="2024" max="2100" value="<?=View::e($profile['year']??date('Y'))?>"></div>
    ...
    <div class="row">
      <div class="field"><label>Sazba sociálního (%)</label><input type="number" step="0.01" name="social_rate" value="<?=View::e($profile['social_rate']??0)?>"></div>
      <div class="field"><label>Sazba zdravotního (%)</label><input type="number" step="0.01" name="health_rate" value="<?=View::e($profile['health_rate']??0)?>"></div>
    </div>
    <div class="row">
      <div class="field"><label>Výdajový paušál (%)</label><input type="number" step="0.01" min="0" max="100" name="expense_lump_rate" value="<?=View::e((float)($profile['expense_lump_rate']??0.6)*100)?>"></div>
      <div class="field"><label>Daňová sleva / rok (Kč)</label><input type="number" step="0.01" name="tax_credit" value="<?=View::e($profile['tax_credit']??0)?>"></div>
    </div>
    <div class="row">
    ...
    <div class="card" style="background:#f8fafc">
      ...
      <div class="row">
        <div class="field"><label>Měsíční paušální daň (Kč)</label><input type="number" step="0.01" name="flat_monthly_tax" value="<?=View::e($profile['flat_monthly_tax']??0)?>"></div>
        <div class="field"><label>Měsíční sociální (Kč)</label><input type="number" step="0.01" name="flat_social_monthly" value="<?=View::e($profile['flat_social_monthly']??0)?>"></div>
      </div>
      <div class="field"><label>Měsíční zdravotní (Kč)</label><input type="number" step="0.01" name="flat_health_monthly" value="<?=View::e($profile['flat_health_monthly']??0)?>"></div>
...
<div class="card" style="margin-top:16px">
  ...
  <p class="sub" style="margin:0"><?=View::e($estimate['rule_date']??'Výpočet je orientační.')?></p>
</div>
